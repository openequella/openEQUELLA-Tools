...
public class EQUELLASOAP {
	...
	/**
	 * Search for items on the EQUELLA server. Consult the SOAP API documentation
	 * for more information on the values of the parameters and return result.
	 */
	public XMLWrapper searchItems(String query, String where, boolean onlyLive, int sortType, boolean reverseSort,
		...
		try {
			return new XMLWrapper(
					client.searchItems(query, null, where, onlyLive, sortType, reverseSort, offset, maxResults));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	/**
	 * Get item from the EQUELLA server.
	 */
	public XMLWrapper getItem(String uuid, int version) {
		try {
			return new XMLWrapper(client.getItem(uuid, version, ""));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	// Returns the uuid.
	public String addUser(String userId, String username, String password, String firstName, String lastName,
			String email) {
		try {
			String result = client.addUser(userId, username, password, firstName, lastName, email);
	 ...
	 * until you call the saveItem method. The item XML will be initialised with a
	 * new UUID and a new staging ID where attachments can be uploaded to.
	 */
	 ...
	 * this, you must either use {@link #editItem(String, int, boolean) editItem} or
	 * {@link #newItem(String) newItem} and use the XML returned by these methods to
	 * pass in as the item parameter.
	 */
	...
	/**
	 * Upload a file into the staging area. Note that this does not attach the file
	 * to your item! To link the file to the item you need to add an attachment node
	 * to the item XML. Consult the SOAP API documentation for the format of the
	 * attachment nodes.
	 */
	...
	/**
	 * Generates a token that is valid for 30 minutes. This should be appended to
	 * URLs so that users are not forced to log in to view content. E.g.
	 ...
	 * 
	 * In the example above, if fred.smith is a valid username on the EQUELLA server
	 * he will be automatically logged into the system so that he can view item
	 * 619722b1-22f8-391a-2bcf-46cfaab36265/1 (provided he has the permissions to do
	 * so).
	 * 
	 * Note that to use this functionality, the Shared Secrets user management
	 * plugin must be enabled (see User Management in the EQUELLA Administration
	 * Console) and a shared secret must be configured.
	 * 
	 * @param username
	 ...
	 * @return A token that can be directly appended to a URL (i.e. it is already
	 *         URL encoded) E.g. URL = URL + "?token=" +
