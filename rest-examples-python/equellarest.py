...
class EquellaRest:
    ...
    def _call(
        ...
        headers = {
            "X-Authorization": "access_token=%s" % (self._oauthToken),
    ...
    def newItem(self, item, draft=False, staging=None):
        ...
        if staging is not None:
            args["file"] = staging
        return self._call(
            path="api/item", args=args, data=item, method="POST", forceRedirect=True
        )
    ...
    def editItem(self, uuid, version, item, submit=True, staging=None):
        ...
        if staging is not None:
            args["file"] = staging
        return self._call(
            path="api/item/%s/%s/" % (uuid, version), args=args, data=item, method="PUT"
        )
    ...
    def uploadFile(self, staging, name, localFile, fileSize):
        args = {"append": "false"}
        return self._call(
            path="api/file/%s/content/%s" % (staging, name),
            args=args,
            data=localFile,
            method="PUT",
            isJson=False,
